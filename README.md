...
If you are using a Switchkey account, you must set the provider context globally. Move to step 4 if you are not using switchkey.
```bash
...
**Issue:** If your source configuration is old (e.g., created in 2018), cloning its rules into a modern property often results in a `400 Bad Request` regarding `json-schema-invalid`. This is usually because behaviors like `breadcrumbs` exist in the rules but aren't recognized by ancient schema versions.
